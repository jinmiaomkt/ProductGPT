...
from dataset3 import TransformerDataset, causal_mask_square, load_json_dataset, causal_mask_rectangular

from model3 import build_transformer
from config3 import get_config, get_weights_file_path, latest_weights_file_path
...
def train_model(config):
    ...
    def cal_loss_loader(dataloader, model, device, loss_fn, num_batches = None):
        ...
        for i, batch in enumerate(dataloader):
            if i < num_batches:
                ...
                encoder_output = model.encode(source_input, long_long_self_mask, lto_input, short_long_cross_mask) # (B, seq_len, d_model)
                decoder_output = model.decode(encoder_output, short_short_self_mask, target_input, short_short_cross_mask) # (B, seq_len, d_model)
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            ...
            # Run the tensors through the encoder, decoder and the projection layer
            encoder_output = model.encode(source_input, long_long_self_mask, lto_input, short_long_cross_mask) # (B, seq_len, d_model)
            # print('target input', target_input)
            decoder_output = model.decode(encoder_output, short_short_self_mask, target_input, short_short_cross_mask) # (B, seq_len, d_model)
            proj_output = model.project(decoder_output) # (B, seq_len, vocab_size)
