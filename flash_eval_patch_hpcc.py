#!/usr/bin/env python3
"""Self-contained Flash-attention adapter for unified_model_eval_hpcc.py."""
...
FEATURE_COLS = [
    ...
    "GenderFemale", "GenderMale", "CountryLiYue", "CountryDaoQi",
...
def load_feature_tensor(xls_path: Path) -> torch.Tensor:
    ...
    arr = np.zeros((MAX_TOKEN_ID + 1, len(FEATURE_COLS)), dtype=np.float32)
...
def load_calibrator_from_path(
    cal_path: Path, device: torch.device
) -> Optional[VectorScaling]:
    if not cal_path.is_file():
        return None
    calibrator = VectorScaling(n_classes=9).to(device)
    state = torch.load(cal_path, map_location=device)
    calibrator.a.data = state["a"].to(device)
    calibrator.b.data = state["b"].to(device)
