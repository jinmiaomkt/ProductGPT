# hP_tuning4_index_git.py   – with unique MASTER_PORT per worker
# -------------------------------------------------------------------------
import multiprocessing as mp
mp.set_start_method("spawn", force=True)

import itertools, json, os, socket                     # ← added socket
...
HP_GRID = list(itertools.product(
    ctx_window_values, d_model_values, d_ff_values,
    N_values, num_heads_values, lr_values, weight_values))
...
def run_one_experiment(params):
    ...
    # ---------- UNIQUE MASTER_PORT to avoid EADDRINUSE ------------------
    ...
    if ckpt_path.exists() and upload_to_s3(
            ckpt_path, bucket, f"{prefix}FullProductGPT/checkpoints/{ckpt_path.name}", s3):
        ckpt_path.unlink()

    if upload_to_s3(metrics_path, bucket,
                    f"{prefix}FullProductGPT/metrics/{metrics_path.name}", s3):
        metrics_path.unlink()
