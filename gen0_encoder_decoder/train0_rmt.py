...
# from model0_vanilla import build_transformer
...
# Import dataset and model building functions (assume these have been revised accordingly)
from dataset0_rmt import TransformerDataset, load_json_dataset  # Revised TransformerDataset now returns segments & memory-aware masks
from model0_rmt import build_transformer
from config0 import get_config, get_weights_file_path, latest_weights_file_path

# --------------------------
# Helper functions for tokenizers
# --------------------------
...
def build_tokenizer(data, key, vocab_size):
    """
    Build a WordLevel tokenizer that splits only on whitespace.
    """
    # Use WordLevel model (no subword merges)
    tokenizer = Tokenizer(models.WordLevel(unk_token="[UNK]"))
    tokenizer.pre_tokenizer = pre_tokenizers.Whitespace()
    trainer = trainers.WordLevelTrainer(
        vocab_size=vocab_size,
...
def get_dataloaders(config):
    ...
    # Build tokenizers using the train data.
    tokenizer_src = build_tokenizer(train_data, "Item", vocab_size=config['vocab_size_src'])
    tokenizer_tgt = build_tokenizer(train_data, "Decision", vocab_size=config['vocab_size_tgt'])
    
    # Create datasets. Note: Pass mem_len to the TransformerDataset.
    train_dataset = TransformerDataset(
        train_data, tokenizer_src, tokenizer_tgt, 
        config['seq_len_src'], config['seq_len_tgt'], 
        config['num_heads'], config['source_rate'],
        config['mem_len_src'], config['mem_len_tgt'],  # NEW: number of memory tokens per segment
        pad_token=0, sos_token=7, eos_token=8
    )
    val_dataset   = TransformerDataset(
        val_data, tokenizer_src, tokenizer_tgt, 
        config['seq_len_src'], config['seq_len_tgt'], 
        config['num_heads'], config['source_rate'],
        config['mem_len_src'], config['mem_len_tgt'], # NEW: pass mem_len
        pad_token=0, sos_token=7, eos_token=8
    )
...
def get_model(config, vocab_src_len, vocab_tgt_len):
    model = build_transformer(
        vocab_src_len, vocab_tgt_len, config["seq_len_src"], config['seq_len_tgt'], d_model=config['d_model'], N=config['N'], h=config['num_heads'], dropout=config['dropout'], d_ff=config['d_ff'], num_memory_tokens_source=config['mem_len_src'], num_memory_tokens_target=config['mem_len_tgt'], layer_dropout_rate=config['dropout']
    )
    return model


# build_transformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, d_model: int, N: int, h: int,dropout: float, d_ff: int, num_memory_tokens: int,layer_dropout_rate: float = 0.1)
...
def train_model(config):
    # Set up the device.
    device_str = "cuda" if torch.cuda.is_available() else "mps" if (hasattr(torch, "has_mps") and torch.has_mps) or torch.backends.mps.is_available() else "cpu"
    ...
    # Helper: calculate loss over a dataloader (process segments sequentially)
    def cal_loss_loader(dataloader, model, device, loss_fn, num_batches=None):
        total_loss = 0
        if len(dataloader) == 0:
            return float('nan')
        ...
        for i, batch in enumerate(dataloader):
            ...
            # Get segmented inputs.
            source_segments = batch['source_segments'].to(device)         # (B, S, seq_len_src)
            target_input_segments = batch['target_input_segments'].to(device) # (B, S, seq_len_tgt)
            label_segments = batch['label_segments'].to(device)               # (B, S, seq_len_tgt)
            B = source_segments.size(0)

            encoder_masks = [mask.to(device).expand(B, -1, -1) for mask in batch['long_long_self_masks']]
            decoder_masks = [mask.to(device).expand(B, -1, -1) for mask in batch['short_short_self_masks']]
            cross_masks   = [mask.to(device).expand(B, -1, -1) for mask in batch['short_long_cross_masks']]

            encoder_memory = None
            decoder_memory = None
            loss_accum = 0.0
            num_segments = source_segments.size(1)
            for seg in range(num_segments):
                seg_source = source_segments[:, seg, :]        # (B, seq_len_src)
                seg_enc_mask = encoder_masks[seg]                # (B, 1, mem_len+seq_len_src)
                enc_output, encoder_memory = model.encode(seg_source, seg_enc_mask, encoder_memory)
                seg_target = target_input_segments[:, seg, :]    # (B, seq_len_tgt)
                seg_dec_mask = decoder_masks[seg]                # (B, 1, mem_len+seq_len_tgt)
                seg_cross_mask = cross_masks[seg]                # (B, num_heads, seq_len_tgt, ?)
                dec_output, decoder_memory = model.decode(enc_output, seg_target, seg_dec_mask, seg_cross_mask, decoder_memory)
                proj_output = model.project(dec_output)          # (B, seq_len_tgt, vocab_size)
                seg_label = label_segments[:, seg, :]            # (B, seq_len_tgt)
                seg_loss = loss_fn(proj_output.view(-1, tokenizer_tgt.get_vocab_size()),
                                   seg_label.view(-1))
                loss_accum += seg_loss
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            ...
            optimizer.step()
            optimizer.zero_grad(set_to_none=True)
            tokens_seen += source_segments.numel()
            global_step += 1

            batch_iterator.set_postfix({"loss": f"{total_loss.item():6.3f}"})
