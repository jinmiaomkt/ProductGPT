...
from model0_re import build_transformer
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            ...
            if global_step % config['eval_freq'] == 0:
                ...
                with torch.no_grad():
                    train_loss = cal_loss_loader(train_dataloader, quantized_model, device, loss_fn)
                    val_loss = cal_loss_loader(val_dataloader, quantized_model, device, loss_fn)
        ...
        torch.save({
            'epoch': epoch,
            'qat_model_state_dict': model.state_dict(),  # QAT model state (still trainable)
            'quantized_model_state_dict': quantized_model.state_dict(),  # For deployment
