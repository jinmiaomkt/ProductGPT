...
class EncoderBlock(nn.Module):
    def __init__(self, features: int, self_attention_block: CausalPerformer, feed_forward_block: FeedForwardBlock, dropout: float) -> None:
        super().__init__()
...
class Transformer(nn.Module):
    ...
    def __init__(self, encoder: Encoder, decoder: Decoder, src_embed: SpecialPlusFeatureLookup, tgt_embed: InputEmbeddings, src_pos: PositionalEncoding, tgt_pos: PositionalEncoding, projection_layer: ProjectionLayer) -> None:
        super().__init__()
...
def build_transformer(src_vocab_size: int, 
    ...
    for _ in range(N):
        ...
        decoder_cross_attention_block = CausalPerformer(d_model, h, dropout, kernel_type, block_size_h=1, block_size_w=10)
        feed_forward_block = FeedForwardBlock(d_model, d_ff, dropout)
