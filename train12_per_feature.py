...
from dataset12_per import TransformerDataset, load_json_dataset
from model12_per_feature import build_transformer
...
class FocalLoss(nn.Module):
    def __init__(self, gamma=2.0, reduction='mean', ignore_index=None):
        ...
        :param reduction: 'mean' (default), 'sum', or 'none'.
    ...
    def forward(self, inputs, targets):
        ...
        :param targets: True token indices, shape (batch_size, seq_len).base-gacha
        :return: Loss value.
        """
...
def get_all_sentences(data, key):
    """
    Convert each data[row][key] to a string or space-joined string of ints.
    """
    for row in data:
        val = row[key]
...
def get_dataloaders(config):
    ...
    train_dataset = TransformerDataset(
        train_data,
        tokenizer_src, tokenizer_tgt, tokenizer_lto,
        config['seq_len_src'], config['seq_len_tgt'], config['seq_len_lto'],
        config['num_heads'], config['source_rate'], config['lto_rate']
    )
...
def get_model(config, vocab_src_len, vocab_tgt_len, vocab_lto_len, feature_tensor, special_token_ids):
    return build_transformer(
        vocab_src_len, vocab_tgt_len, vocab_lto_len,
        config["seq_len_src"], config['seq_len_tgt'], config['seq_len_lto'],
