...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, limited_time_offer, long_long_self_mask, short_long_cross_mask):
        ...
        limited_time_offer = self.residual_connections[1](limited_time_offer, lambda lto_norm: 
                                                          self.cross_attention_block(lto_norm, x, x, short_long_cross_mask))
...
class Encoder(nn.Module):
    ...
    def forward(self, x, limited_time_offer, long_long_self_mask, short_long_cross_mask):
        for layer in self.layers:
            limited_time_offer = layer(x, limited_time_offer, long_long_self_mask, short_long_cross_mask)
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, encoder_output, short_short_self_mask, short_short_cross_mask):
        x = self.residual_connections[0](x, lambda x_norm: self.self_attention_block(x_norm, x_norm, x_norm, short_short_self_mask))
        x = self.residual_connections[1](x, lambda x_norm: self.cross_attention_block(x_norm, encoder_output, encoder_output, short_short_cross_mask))
...
class Decoder(nn.Module):
    ...
    def forward(self, x, encoder_output, short_short_self_mask, short_short_cross_mask):
        for layer in self.layers:
            x = layer(x, encoder_output, short_short_self_mask, short_short_cross_mask)
        return self.norm(x)
...
class Transformer(nn.Module):
    ...
    def encode(self, src: torch.Tensor, long_long_self_mask: torch.Tensor, lto: torch.Tensor, short_long_cross_mask: torch.Tensor):
        # (batch, seq_len, d_model)
        src = self.src_embed(src)
        src = self.src_pos(src)
        lto = self.lto_embed(lto)
        lto = self.lto_pos(lto)
        return self.encoder(src, lto, long_long_self_mask, short_long_cross_mask)
    
    def decode(self, encoder_output: torch.Tensor, short_short_self_mask: torch.Tensor, tgt: torch.Tensor, short_short_cross_mask: torch.Tensor):
        # (batch, seq_len, d_model)
        tgt = self.tgt_embed(tgt)
        tgt = self.tgt_pos(tgt)
        return self.decoder(tgt, encoder_output, short_short_self_mask, short_short_cross_mask)
