...
# --ckpt <PRODUCTGPT_OUTPUT>/FullProductGPT_featurebased_performerfeatures16_dmodel32_ff32_N8_heads4_lr0.0001_w4.pt \
...
def get_config():
    cfg = _raw_config()
    cfg["window_size"] = cfg["ctx_window"] * cfg["ai_rate"]
    cfg["block_size"] = 16 * cfg["ai_rate"]
...
def get_weights_file_path(config, tag: str) -> str:
    ...
    uid = (f"ai_rate{config['ai_rate']}_ctx{config['ctx_window']}_dmodel{config['d_model']}_ff{config['d_ff']}"
