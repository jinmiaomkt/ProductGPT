...
────────────────────────────────────────────────────────────
* 80-10-10 split  ·  rich metrics (all / cur-STOP / after-STOP / transition)
* best checkpoint + metrics JSON uploaded to:
      s3://productgptbucket/GRU/checkpoints/<file>.pt
      s3://productgptbucket/GRU/metrics/<file>.json
"""
...
S3_PREFIX = "GRU"            # top-level bucket prefix
...
def evaluate(loader, model, device, loss_fn):
    ...
    with torch.no_grad():
        for xb, yb in loader:
            ...
            true_p = probs[torch.arange(len(flat_labels)),
                           flat_labels.clamp(max=C-1)]
            tot_ppl += torch.exp(-torch.log(true_p + 1e-9).mean()).item()
...
def run_one(params):
    ...
    metrics = {
        "hidden_size": hidden, "lr": lr, "batch_size": bs,
        **best_metrics,
