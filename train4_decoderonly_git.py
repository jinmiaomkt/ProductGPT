...
#   • PairwiseRevenueLoss (- E|Rᵢ – Rⱼ|)
#   • Four evaluation slices:  all / STOP-cur / after-STOP / transition
#   • No DeepSpeed chatter, JSON metadata identical to the old script
# ----------------------------------------------------------------------
...
def transition_mask(seq: torch.Tensor) -> torch.Tensor:
    """True where seq[t] ≠ seq[t-1]; shape (B, T_decision)."""
...
def _perplexity(logits, targets, pad=0):
    ...
    m = tgt != pad
    return (float("nan") if m.sum() == 0
            else torch.exp(F.nll_loss(lp2d[m], tgt[m], reduction="mean")).item())
...
def _loaders(cfg):
    ...
    tr_ds, va_ds, te_ds = random_split(data, [tr, va, n-tr-va], generator=g)
...
# ══════════════════════ 6. evaluation ═══════════════════════════════════
def _evaluate(loader, eng, dev, loss_fn, pad, tok):
    if not loader:
    ...
    with torch.no_grad():
        for b in loader:
            ...
            # ------ forward at decision positions only -----------------
            n_slots = tgt.size(1)                 # ≡ seq_len_tgt
            pos     = torch.arange(tok.token_to_id("[EOS]")-1,   # step-1
                                   tok.token_to_id("[EOS]")-1 + cfg["ai_rate"]*n_slots,
                                   cfg["ai_rate"], device=dev)
            logits  = eng(x)[:, pos, :]           # (B, n_slots, V)

            # ------ mask transitions for the loss ----------------------
            tgt_loss = tgt.clone()
            tgt_loss[transition_mask(tgt)] = pad

            L_loss += loss_fn(logits, tgt_loss).item()
            L_ppl  += _perplexity(logits, tgt_loss, pad)
            ...
            prev_dec   = F.pad(tgt, (1,0), value=-1)[:, :-1]
            m_after_stop.append(flat(prev_dec == 9))
    ...
    return (L_loss/len(loader), L_ppl/len(loader),
            _subset(P,L,PR, mask_all),
            _subset(P,L,PR, mask_stop_cur),
            _subset(P,L,PR, mask_after_stop),
            _subset(P,L,PR, mask_tr))
