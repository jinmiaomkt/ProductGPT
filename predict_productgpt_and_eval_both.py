...
def parse_args():
    ...
    p.add_argument("--ai-rate", type=int, default=15)
...
def phase_split_uids_exact(
    *,
    ...
    if overlap:
        raise RuntimeError(f"Split overlap: {list(overlap)[:5]}")
...
def compute_multiclass_metrics(
    ...
    """
    Compute 6 aggregate multiclass metrics for a single (group, split) bucket.
    """
...
def compute_perclass_metrics(
    ...
    p_arr : (N, 9) float array, predicted probabilities
    ...
    results = {}
    for c_idx, c in enumerate(NINE_CLASSES):              # c in [1..9]
        support     = int((y_arr == c).sum())
        y_bin_c     = y_bin[:, c_idx]
        p_c         = p_arr[:, c_idx]
        y_hat_bin_c = (y_hat == c).astype(int)

        # AUC OvR — needs both classes present
        ...
        # AUPRC — undefined when no positive examples
        if y_bin_c.sum() == 0:
            auprc = np.nan
        else:
            auprc = average_precision_score(y_bin_c, p_c)

        # Binary F1 for this class (hard prediction via argmax)
...
def main():
        ...
        for spl in SPLIT_ORDER:
            ...
            for c, cm in pc.items():
                perclass_rows.append({
                    "Group": grp, "Split": spl, "Class": c, **cm
                })

    perclass_df = pd.DataFrame(perclass_rows)
    # ─────────────────────────────────────────────────────────────────────────

    # ── Column layouts for individual panels ─────────────────────────────────
    MULTI_METRICS    = ["MacroOvR_AUC", "MacroAUPRC", "MacroF1", "MCC", "LogLoss", "Top2Acc"]
    PERCLASS_METRICS = ["AUC_OvR", "AUPRC", "F1", "Support"]
    BIN_METRIC_COLS  = ["BinaryAUC", "BinaryAUPRC"]

    # All metrics that can appear in the combined report, in display order.
    # Each section populates only its own columns; the rest are NaN.
    ...
    # Also build a full binary table (all 6 tasks) for the combined report
    ...
    def build_combined_report(group_name: str) -> pd.DataFrame:
        ...
        One flat DataFrame per group with three stacked sections:
          ...
          [1] Aggregate 9-class metrics  (1 row  — "Aggregate")
          [2] Per-class OvR metrics      (9 rows — "Class 1" … "Class 9")
          [3] Binary task metrics        (6 rows — one per BIN_TASKS entry)

        Columns are a MultiIndex (Metric, Split).  Cells not relevant to a
        section are left as NaN so the table reads cleanly.
        """
        rows = {}

        # ── Section 1: aggregate 9-class ─────────────────────────────────────
        ...
        # ── Section 3: binary tasks (all 6) ──────────────────────────────────
        bin_sub = all_binary[all_binary["Group"] == group_name].copy()
        # Pretty-print task names: use PAPER_AUC_TASKS where available, else raw
        bin_sub["Label"] = bin_sub["Task"].map(
            lambda t: PAPER_AUC_TASKS.get(t, t)
        )
        ...
        # ── Assemble ──────────────────────────────────────────────────────────
        # Row order: Aggregate → Class 1..9 → binary tasks
        class_labels  = [f"Class {c}" for c in NINE_CLASSES]
        binary_labels = (
            bin_sub[["Label"]].drop_duplicates()["Label"].tolist()
        )
        # Preserve stable order (avoid set reordering)
        seen = set()
        ordered_labels = []
        ...
        # Section separator: insert blank NaN rows between sections
        agg_rows  = ["Aggregate"]
        ...
        parts = []
        if agg_rows:
            parts += [tbl.loc[agg_rows], sep]
        if class_rows:
            parts += [tbl.loc[class_rows], sep]
        if bin_rows:
            parts += [tbl.loc[bin_rows]]
    ...
    csv_map = {
        ...
        # Combined reports — one file per group, everything in one sheet
