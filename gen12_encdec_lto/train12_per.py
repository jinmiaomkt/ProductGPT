...
from model12_per import build_transformer
...
def train_model(config):
   ...
   # -------------------------
    ...
    # total_steps = config.get('num_epochs') * len(train_loader)
    # peak_lr = config['lr']
    # min_lr = config.get('min_lr', 1e-6)

    # scheduler = WarmupCosineDecayLR(optimizer, warmup_steps, total_steps, peak_lr, min_lr)
    # scheduler = WarmupLR(optimizer, warmup=warmup_steps, total_num_steps = total_steps)
