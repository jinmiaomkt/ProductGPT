...
from model12_vanilla import build_transformer
...
# def greedy_decode(model, source, lto, long_long_self_mask, short_long_cross_mask, short_short_self_mask, short_short_cross_mask, tokenizer_tgt, max_len, device):
...
#             model_out = greedy_decode(model, source_input, lto_input, long_long_self_mask, short_long_cross_mask, short_short_self_mask, short_short_cross_mask, tokenizer_tgt, max_len, device)
...
def get_model(config, vocab_src_len, vocab_tgt_len, vocab_lto_len):
    model = build_transformer(vocab_src_len, vocab_tgt_len, vocab_lto_len, config["seq_len_src"], config['seq_len_tgt'], config['seq_len_lto'], d_model=config['d_model'], N = config['N'], h=config['num_heads'], dropout=config['dropout'], d_ff=config['d_ff'])
    return model
...
def train_model(config):
    ...
    def cal_loss_loader(dataloader, model, device, loss_fn, num_batches = None):
        ...
        for i, batch in enumerate(dataloader):
            if i < num_batches:
                source_input = batch['source_input'].to(device) # (b, seq_len_src)
                target_input = batch['target_input'].to(device) # (B, seq_len_tgt)
                lto_input = batch['lto_input'].to(device) # (B, seq_len_lto)

                src_src_self_mask = batch['src_src_self_mask'].to(device) # (B, 1, seq_len_src, seq_len_src)
                tgt_tgt_self_mask = batch['tgt_tgt_self_mask'].to(device) # (B, 1, seq_len_tgt, seq_len_tgt)
                lto_src_cross_mask = batch['lto_src_cross_mask'].to(device) # (B, 1, seq_len_lto, seq_len_src)
                tgt_lto_cross_mask = batch['tgt_lto_cross_mask'].to(device) # (B, 1, seq_len_tgt, seq_len_lto)

                # Run the tensors through the encoder, decoder and the projection layer
                encoder_output = model.encode(source_input, src_src_self_mask, lto_input, lto_src_cross_mask) # (B, seq_len_lto, d_model)
                decoder_output = model.decode(encoder_output, tgt_tgt_self_mask, target_input, tgt_lto_cross_mask) # (B, seq_len_tgt, d_model)
                proj_output = model.project(decoder_output) # (B, seq_len_tgt, vocab_size)

                # Compare the output with the label
                label = batch['label'].to(device) # (B, seq_len_tgt)
