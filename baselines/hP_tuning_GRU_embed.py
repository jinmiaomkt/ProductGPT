#!/usr/bin/env python3
"""
hP_tuning_GRU_embed.py

GRU baseline with the SAME input representation as ProductGPT:
...
INPUT_DIM    = 15        # tokens per block (decision + 4 LTO + 10 products)
...
LOCAL_TMP = Path("/home/ec2-user/tmp_gru_embed")
...
class TokenSequenceDataset(Dataset):
    """Reads the same JSON as the transformer, tokenizes to IDs."""
    def __init__(self, json_path: str):
        ...
        self.x, self.y = [], []
        for row in rows:
            # Tokenize aggregate input (same as transformer)
            ...
            # Tokenize decisions (same as transformer)
            dec = row["Decision"]
...
# ═════════ 3. GRU model with embedding ═════════
class GRUEmbedClassifier(nn.Module):
    """
    GRU that takes the SAME token ID input as the transformer.
    Uses SpecialPlusFeatureLookup for embedding, then GRU over the sequence.
    ...
    def forward(self, x):
        # x: (B, seq_len_ai) token IDs — same input as transformer
        emb = self.embed(x)              # (B, seq_len_ai, d_model)
        out, _ = self.gru(emb)           # (B, seq_len_ai, hidden_size)
        logits = self.fc(out)            # (B, seq_len_ai, NUM_CLASSES)

        # Extract decision positions (same as transformer)
...
def evaluate(loader, model, device, loss_fn):
    ...
    with torch.no_grad():
        for xb, yb in loader:
            ...
            logits = model(xb)               # (B, n_decisions, C)
            B, T, C = logits.shape
...
def run_one(params):
    ...
    model = GRUEmbedClassifier(d_model, hidden, feat_tensor, AI_RATE).to(dev)
    ...
    ckpt_path = LOCAL_TMP / f"gru_embed_{uid}.pt"
...
def sweep(max_workers=1):
    ...
    summary_path.write_text(json.dumps({"all_results": _json_safe(all_results), "best": _json_safe(best)}, indent=2))
