...
from train2_decoderonly_performer_index_aws  import train_model
import numpy as np
...
def run_one(params):
    ...
    if not json_path.exists():
        ...
        if ckpt.exists() and s3_put(ckpt, bucket,
            f"LP_ProductGPT/performer/Index/checkpoints/{ckpt.name}", s3):
            ckpt.unlink()

        if s3_put(json_path, bucket,
                f"LP_ProductGPT/performer/Index/metrics/{json_path.name}", s3):
            json_path.unlink()
