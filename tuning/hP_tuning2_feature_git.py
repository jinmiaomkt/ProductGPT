...
def run_one_experiment(params):
    ...
    config["lp_rate"] = 5                          # <<<< FIX
    config["seq_len_lp"] = config["lp_rate"] * config["seq_len_tgt"]
    ...
    if ckpt.exists() and s3_put(ckpt, bucket, f"LP_ProductGPT/performer/Feature/checkpoints/{ckpt.name}", s3):
         ckpt.unlink()

    if s3_put(json_path, bucket, f"LP_ProductGPT/performer/Feature/metrics/{json_path.name}", s3):
        json_path.unlink()
