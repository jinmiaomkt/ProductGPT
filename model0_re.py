...
class ReformerAttentionBlock(nn.Module):
    def __init__(self, d_model: int, h: int, dropout: float):
        super().__init__()
        self.reformer_attention = LSHSelfAttention(
            dim=d_model,
            heads=h,
            bucket_size= math.ceil(math.sqrt(d_model)),
            n_hashes=8,  # Number of hash rounds
            causal=True,
            dropout=dropout
        )

    def forward(self, q, k, v, mask=None):
        return self.reformer_attention(q)


class EncoderBlock(nn.Module):

    def __init__(self, features: int, self_attention_block: ReformerAttentionBlock, feed_forward_block: FeedForwardBlock, dropout: float) -> None:
        super().__init__()
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, features: int, self_attention_block: ReformerAttentionBlock, cross_attention_block: ReformerAttentionBlock, feed_forward_block: FeedForwardBlock, dropout: float) -> None:
        super().__init__()
        self.self_attention_block = self_attention_block
        self.cross_attention_block = cross_attention_block
        self.feed_forward_block = feed_forward_block
...
def build_transformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, d_model: int, N: int, h: int, dropout: float, d_ff: int) -> Transformer:
    ...
    encoder_blocks = []
    for _ in range(N):
        encoder_self_attention_block = ReformerAttentionBlock(d_model, h, dropout)
    ...
    decoder_blocks = []
    for _ in range(N):
        decoder_self_attention_block = ReformerAttentionBlock(d_model, h, dropout)
        decoder_cross_attention_block = ReformerAttentionBlock(d_model, h, dropout)
        feed_forward_block = FeedForwardBlock(d_model, d_ff, dropout)
