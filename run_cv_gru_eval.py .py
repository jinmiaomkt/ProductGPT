...
10-fold (configurable) CV orchestrator for the GRU pipeline.

What this does per fold k:
  • creates train/val/test UID sets (test = fold k, val = fold (k+1) mod K)
  • uploads UID lists (val/test) to S3 for exact-match reproducibility
  • trains a GRU on the train set, early-stopping on the val set
  • saves the best checkpoint locally and (optionally) uploads to S3
  • runs predict_gru_and_eval.py for that fold, pointing it at the fold UID lists
  • evaluation CSVs + (optional) preds land under .../eval/fold{k}/ on S3
  • prints a compact per-fold training summary and writes a CV summary CSV to S3

Requirements:
  - predict_gru_and_eval.py must be reachable/executable
  - Training data JSON is a list of records with keys:
      uid, AggregateInput[0] (space-separated features), Decision[0] (space-separated labels)
  - IAM role or AWS credentials present on the machine for S3 ops
...
# ------------------------- S3 helpers -------------------------
try:
    import boto3  # optional (falls back to aws CLI)
except Exception:
    boto3 = None
...
def train_one_fold(
    ...
    # datasets
    ds_tr = GRUSequenceDataset(data_json, input_dim, uids_train)
    ds_va = GRUSequenceDataset(data_json, input_dim, uids_val)
    ds_te = GRUSequenceDataset(data_json, input_dim, uids_test)
    ...
    # model/opt
    model = GRUClassifier(input_dim=input_dim, hidden_size=hidden_size, num_classes=10).to(device)
...
def parse_args():
    ...
    p.add_argument("--predict-eval-script", required=True, help="Path to predict_gru_and_eval.py")
    ...
    p.add_argument("--s3-prefix", required=True, help="Base prefix, e.g. GRU/CV/h128_lr0.001_bs4 (no s3://, bucket separate)")
...
def main():
    ...
    for k in range(args.num_folds):
        ...
        # 2) train GRU on this fold
        ckpt_local = Path(tempfile.gettempdir()) / f"gru_fold{k}_h{args.hidden_size}_lr{args.lr}_bs{args.train_batch_size}.pt"
        ...
        if args.upload-ckpt if False else False:  # guard for syntax
            pass
        if args.upload_ckpt:
        ...
        preds_local = Path(tempfile.gettempdir()) / f"gru_fold{k}_preds.jsonl.gz"
        cmd = [
        ...
        if rc != 0:
            raise RuntimeError(f"predict_gru_and_eval.py failed for fold {k} (rc={rc})")
    ...
    local_csv = Path("/tmp") / f"gru_cv_train_summary_{ts}.csv"
