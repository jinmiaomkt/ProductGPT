...
from model2_decoderonly_index_performer   import build_transformer
...
def _make_loaders(cfg):
    ...
    def mk(split):
        return TransformerDataset(
            ...
            cfg["seq_len_lp"], cfg["seq_len_tgt"],
            cfg["num_heads"], cfg["lp_rate"], pad_token=0)
...
# ══════════════════════ 6.  EVALUATION ══════════════════════════════════
def _evaluate(loader, eng, dev, loss_fn, pad, tok, lp_rate):
    if not loader:
    ...
    with torch.no_grad():
        for b in loader:
            x   = b["LTO_PreviousDecision"].to(dev)       # (B, seq_len_ai)
            tgt = b["label"].to(dev)                 # (B, seq_len_tgt)

            seq_len = x.size(1)
            pos = torch.arange(lp_rate -1,
                            seq_len, 
                            lp_rate,
                            device=dev)
...
def train_model(cfg):
    ...
    ckpt_path = Path(cfg["model_folder"]) / f"LP_ProductGPT_{uid}.pt"
    json_path = ckpt_path.with_suffix(".json")

    s3      = _s3_client()
    bucket  = cfg["s3_bucket"]
    ck_key  = f"LP_ProductGPT/performer/IndexBased/checkpoints/{ckpt_path.name}"
    js_key  = f"LP_ProductGPT/performer/IndexBased/metrics/{json_path.name}"
    ...
    for ep in range(cfg["num_epochs"]):
        ...
        for b in tqdm(tr, desc=f"Ep {ep:02d}", leave=False):
            x   = b["LTO_PreviousDecision"].to(dev)
            tgt = b["label"].to(dev)               # (B, n_slots)

            # seq_len = logits.size(1)
            pos = torch.arange(cfg["lp_rate"]-1,
                            cfg["seq_len_lp"],
                            cfg["lp_rate"],
                            device=dev)
            assert pos.numel() == tgt.size(1), (
        ...
        v_loss,v_ppl,v_all,v_stop,v_after,v_tr = _evaluate(
            va, eng, dev, loss_fn, pad_id, tok_tgt, cfg["lp_rate"])
    ...
    if ckpt_path.exists():
        ...
        t_loss,t_ppl,t_all,t_stop,t_after,t_tr = _evaluate(
            te, eng, dev, loss_fn, pad_id, tok_tgt, cfg["lp_rate"])
...
if __name__ == "__main__":
    cfg = get_config()
    cfg["lp_rate"] = 5
    cfg["seq_len_lp"]  = cfg["lp_rate"] * cfg["seq_len_tgt"]   # = 480
    train_model(cfg)
