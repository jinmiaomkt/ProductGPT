...
Usage:
  python3 unified_model_eval_hpcc.py \\
    --config    model_specs_hpcc.json \\
    ...
    --fold-id   0 \\
    --output-dir /storage/home/jinmiao/ProductGPT/evals/unified_fold0 \\
    --compare-on test

All inputs and outputs are local OMEGA paths. The program refuses to run
without a CUDA allocation unless --allow-cpu is supplied explicitly.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--uids-val",    default="",    help="Local validation UID file")
    p.add_argument("--uids-test",   default="",    help="Local test UID file")
    ...
    p.add_argument(
        "--allow-cpu",
        action="store_true",
        help="Allow CPU evaluation; omit on OMEGA to catch accidental login-node runs.",
    )
    return p.parse_args()
...
def local_file(path_value: str | Path, label: str) -> Path:
    """Resolve a local OMEGA path and fail with a clear message if absent."""
...
def load_uid_set(path_value: str) -> Set[str]:
    if not path_value:
        return set()
    text = local_file(path_value, "UID file").read_text()
    return {ln.strip() for ln in text.splitlines() if ln.strip() and not ln.strip().startswith("#")}


def load_json_local(path_value: str, label: str = "JSON file") -> Any:
    return json.loads(local_file(path_value, label).read_text())
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                if self.calibration == "calibrator" and self.calibrator is not None:
                    ...
                    # Match the training evaluator: analytic correction subtracts
                    # the log class-weight bias before softmax.
                    probs = torch.softmax(
...
def build_uid_to_index_for_mixture(
    ...
    spec          = load_json_local(fold_spec_uri, "fold specification")
...
class MixtureAdapter(BaseAdapter):
    def __init__(self, spec: Dict[str, Any], args: argparse.Namespace):
        ...
        self.calibrator   = None
        self.logit_bias_9 = None
        self.fold_spec = spec.get("fold_spec")
        if not self.fold_spec:
            raise ValueError(
                f"{self.name}: local 'fold_spec' path is required for mixture evaluation"
            )
...
def _load_rnn_state(ckpt_path: Path, device: torch.device) -> dict:
    ...
    if not isinstance(state, dict):
        raise TypeError(f"Unsupported RNN checkpoint type: {type(state).__name__}")
    sd = state.get("model_state_dict", state.get("state_dict", state.get("module", state)))
...
def validate_local_model_paths(models: Sequence[Dict[str, Any]]) -> None:
    """Reject stale S3 URIs and missing model artefacts before inference starts."""
    path_keys = ("ckpt", "calibrator_ckpt", "feat_xlsx", "fold_spec", "train_file")
    problems: List[str] = []
    for spec in models:
        ...
        for key in path_keys:
            ...
            if str(value).startswith("s3://"):
                problems.append(f"{model_name}.{key} is still an S3 URI: {value}")
                continue
            path = Path(os.path.expandvars(str(value))).expanduser()
            if not path.is_file():
                problems.append(f"{model_name}.{key} not found: {path}")
            else:
                spec[key] = str(path)
    if problems:
        raise FileNotFoundError("Local model-path validation failed:\n  - " + "\n  - ".join(problems))
...
def main() -> None:
    ...
    if not torch.cuda.is_available() and not args.allow_cpu:
        raise RuntimeError(
            "CUDA is unavailable. Submit this evaluator as a PBS GPU job. "
            "Use --allow-cpu only for a deliberately small CPU smoke test."
        )

    config_path = local_file(args.config, "model configuration")
    args.data = str(local_file(args.data, "evaluation data"))
    args.labels = str(local_file(args.labels, "label data"))
    if args.uids_val:
        args.uids_val = str(local_file(args.uids_val, "validation UID file"))
    if args.uids_test:
        args.uids_test = str(local_file(args.uids_test, "test UID file"))
    ...
    validate_local_model_paths(models)
    ...
    (output_dir / "run_environment.json").write_text(json.dumps(runtime, indent=2))
